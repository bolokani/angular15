...
export class FooterComponent implements OnInit, OnDestroy {
  ...
  get_footer_contact() {
    this.subscription = this.serverService.post_address(this.server2, 'new_address', { address: 2154 }).subscribe(
      (res: any) => {
        if (res['status'] == 1) {
          if (res['num'] == 1) {
  ...
  get_footer_content() {
    this.subscription = this.serverService.post_address(this.server2, 'new_address', { address: 2163 }).subscribe(
      (res: any) => {
        if (res['status'] == 1) {
